 ...
 *   2. Clicking a template card calls onSelect(def, permissionMode, name).
 *   3. The blank-canvas card calls onSelect(undefined).
...
describe('GalleryNew', () => {
  ...
  it('clicking a template card calls onSelect(def, permissionMode, name)', () => {
    const onSelect = vi.fn();
    render(
      <GalleryNew isOpen onClose={() => {}} templates={TEMPLATES} onSelect={onSelect} />,
    );
    fireEvent.click(screen.getByTestId('gallery-new-template-a'));
    expect(onSelect).toHaveBeenCalledWith(def('planner'), 'acceptEdits', 'Planner');
  });

  it('clicking blank canvas calls onSelect(undefined)', () => {
    const onSelect = vi.fn();
    render(
      <GalleryNew isOpen onClose={() => {}} templates={TEMPLATES} onSelect={onSelect} />,
    );
