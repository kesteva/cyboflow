...
describe('ChatInput — workflow-idle nudge (awaiting_review)', () => {
  ...
  it('renders a read-only model pill from the run\'s pinned model', () => {
    ...
    render(<ChatInput runId={RUN_ID} />);
    expect(screen.getByText('Opus 5.5 · 1M')).toBeInTheDocument();
  });
  ...
  it('shows the fallback model (not the dead pin) when the pinned model is unavailable', () => {
    ...
    expect(screen.getByText('Opus 5.5 · 1M').closest('span')).toHaveAttribute(
      'title',
      'Fable 5.1 · 1M is unavailable — this run uses Opus 5.5 · 1M.',
    );
  });
