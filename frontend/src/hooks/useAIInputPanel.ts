...
export interface UseAIInputPanelOptions {
  onSendMessage: (message: string, options?: Record<string, unknown>, attachedImages?: AttachedImage[], attachedTexts?: AttachedText[]) => Promise<void>;
  onCancel?: () => void;
  disabled?: boolean;
}
...
export const useAIInputPanel = (options: UseAIInputPanelOptions) => {
  ...
  const handleSubmit = useCallback(async (additionalOptions?: Record<string, unknown>) => {
