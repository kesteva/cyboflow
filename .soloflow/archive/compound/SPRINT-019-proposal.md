...
- **Proposed change:**
  ...
  -xcrun notarytool log 0c820130-8bfc-4d58-b825-76f8abf94e40 --apple-id rkesteva@gmail.com --team-id Y7B83UUSAC
...
- **Verdict:** IMPLEMENT
- **Confidence:** medium
- **Reasoning:** Confirmed `docs/signing/builds/0.3.5/FIRST_SIGNED_BUILD_LOG.md:99` contains `--apple-id rkesteva@gmail.com` verbatim while the rest of the signing docs use `<APPLE_ID>` placeholders — the template's existing `<APPLE_ID>` usage at line 83 needs an explicit prose warning to make the convention sticky for future builders; note that the same email already appears in `docs/signing/APPLE_DEVELOPER_SETUP.md:14,22` so this is mitigation, not a hard PII boundary.
