...
- `cyboflowPermissionIpcServer.ts` and its `start()` call site in `main/src/index.ts`. The SDK path does NOT consume the IPC server (in-process `PreToolUse` hook replaces the socket round-trip), but the IPC server is the orchestrator-side endpoint a future IDEA-013 shell hook would talk to. **Executor note for TASK-590:** do NOT remove `cyboflowPermissionIpcServer.start()` from `main/src/index.ts`, and do NOT delete `cyboflowPermissionIpcServer.ts`. Leaving the server wired up (even though the SDK path doesn't drive it) keeps IDEA-013 a one-task pivot — wiring a shell hook to an already-running socket — rather than an un-deletion. The four ready hardening tasks (TASK-303, TASK-580, TASK-581, TASK-582, TASK-583) on this IPC server retain their value under that future pivot.
