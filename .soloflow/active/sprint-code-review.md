...
### Minor (low severity)
- FIND-SPRINT-019-9 — Real Apple ID email `<APPLE_ID>` appears in committed 0.3.5 build evidence while templates use `<APPLE_ID>` placeholders; pattern likely to repeat in every future build record unless the template adds a redaction note.
