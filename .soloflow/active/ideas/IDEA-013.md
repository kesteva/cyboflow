...
open_questions:
  - "RESOLVED 2026-06-01 (GO — user sign-off): Probe H (ship-gating business risk) is accepted. The user has run substantially more than 4 parallel interactive `claude` sessions on their plan in practice without hitting concurrency/rate-limit walls, so the '8 parallel agents on your subscription' value prop is accepted on operator experience. Knowingly-held product assumption (support.claude.com/articles/15036540 blesses interactive terminal/IDE use but is silent on automated/parallel driving); the SDK substrate remains the fallback for heavy/automated load. Original question retained for context: does Anthropic's plan ToS / rate-limiting permit driving N>=4 parallel interactive sessions on a Pro/Max subscription?"
