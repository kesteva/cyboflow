...
// resolveAgentModelAlias / displayAgentModelSelection (the provider-namespacing
// wrapper around resolveModelAlias) are tested alongside their own module at
// main/src/services/panels/__tests__/agentModelContext.test.ts — that wrapper
// stays main-side, so its test can't live under shared/ without pulling a
// main-only module into a shared test.
...
describe('modelContext', () => {
  ...
  describe('applyModelAvailabilityFallback — guarded-model swap', () => {
  ...
  describe('resolveUnavailableDefaultModelFallback — auto-mode classifier default guard', () => {
    ...
    it('returns the resolved Opus fallback when the guarded default is unavailable', () => {
      expect(resolveUnavailableDefaultModelFallback(() => false)).toBe('claude-opus-5-5[1m]');
    });
  ...
  describe('bareModelId — optional availability fallback', () => {
    it('resolves the bare snapshot with no predicate (unchanged legacy behavior)', () => {
      expect(bareModelId('opus')).toBe('claude-opus-5-5');
    ...
    it('falls back to the bare Opus id when a pinned Fable is unavailable', () => {
      expect(bareModelId('fable', () => false)).toBe('claude-opus-5-5');
