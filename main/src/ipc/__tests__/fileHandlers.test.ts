/**
 * Behavioral tests for the destructive/file ops in main/src/ipc/fileOps.ts
 * (createFileOps — backs the `workspaceFiles` tRPC router).
 *
 ...
 * exercised through the ops directly with a stubbed sessionManager.
 *
 * Covered:
...
describe('file path guards (file:read / file:write)', () => {
  ...
  it('file:read rejects a `../` traversal path', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.read({ sessionId: 's1', filePath: '../outside.txt' });
    expect(result.success).toBe(false);
    expect(!result.success && result.error).toBe('Invalid file path');
  });
  ...
  it('file:read rejects an absolute path', async () => {
    ...
    fs.mkdirSync(worktree);
    const ops = opsForWorktree(worktree);
    const result = await ops.read({ sessionId: 's1', filePath: '/etc/passwd' });
    expect(result.success).toBe(false);
    expect(!result.success && result.error).toBe('Invalid file path');
  });
  ...
  it('file:write rejects a `../` traversal path and writes nothing', async () => {
    ...
    const result = await ops.write({ sessionId: 's1', filePath: '../outside.txt', content: 'pwned' });
    expect(result.success).toBe(false);
    expect(!result.success && result.error).toBe('Invalid file path');
  ...
  it('file:read rejects a symlink inside the worktree that points outside it', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.read({ sessionId: 's1', filePath: 'escape.txt' });

    expect(result.success).toBe(false);
    expect(!result.success && result.error).toBe('File path is outside worktree');
  ...
  it('file:read rejects a symlink escaping to a sibling worktree that shares a name prefix', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.read({ sessionId: 's1', filePath: 'leak.txt' });
  ...
  it('file:write rejects a symlink escaping the worktree and does not write through it', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.write({ sessionId: 's1', filePath: 'escape-w.txt', content: 'PWNED' });
  ...
  it('file:write rejects a DANGLING symlink pointing outside the worktree and does not create the target', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.write({ sessionId: 's1', filePath: 'dangling.txt', content: 'PWNED' });
  ...
  it('file:write through a dangling symlink to a not-yet-existing path INSIDE the worktree works', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.write({ sessionId: 's1', filePath: 'dangling-in.txt', content: 'created via link' });
  ...
  it('file:read follows a legitimate symlink that stays within the worktree', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.read({ sessionId: 's1', filePath: 'link.txt' });

    expect(result.success).toBe(true);
    expect(result.success && result.content).toBe('INSIDE\n');
  });
  ...
  it('file:read reads a normal in-worktree file unaffected by the guard', async () => {
    ...
    const ops = opsForWorktree(worktree);
    const result = await ops.read({ sessionId: 's1', filePath: 'note.txt' });

    expect(result.success).toBe(true);
    expect(result.success && result.content).toBe('hello\n');
  });
});
...
describe('file:write nested directory creation', () => {
  it('creates missing parent directories under the worktree', async () => {
    ...
    const result = await ops.write({ sessionId: 's1', filePath: rel, content: 'hello nested' });
...
describe('file:readAtRevision', () => {
  it('returns empty content when the file does not exist at the revision', async () => {
    ...
    const result = await ops.readAtRevision({
      sessionId: 's1',
    ...
    expect(result.success).toBe(true);
    expect(result.success && result.content).toBe('');
  });
  ...
  it('returns the committed content for a file that exists at HEAD', async () => {
    ...
    const result = await ops.readAtRevision({ sessionId: 's1', filePath: 'tracked.txt', revision: 'HEAD' });

    expect(result.success).toBe(true);
    expect(result.success && result.content).toBe('HEAD CONTENT\n');
  });
