...
const FROZEN_LINE_CAPS: Record<string, number> = {
  'orchestrator/mcpServer/mcpQueryHandler.ts': 2457, // steps 1-3 + 9-12: type contract → mcpQueryMessages.ts; workflow-config + global-agent read/propose tools + verify/eval tools + interactive shell/OMP approval hooks + run-bound task/backlog tools + review-queue tools → handlers/; mcp-queue → handlers/globalAgentToolHandlers.ts (TASK-293); backlog projections → backlogProjection.ts
