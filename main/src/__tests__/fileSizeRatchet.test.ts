...
const FROZEN_LINE_CAPS: Record<string, number> = {
  ...
  'index.ts': 6281, // step 4: verify + eval composition → verifyComposition.ts / evalComposition.ts (siblings); seeded-finding reader → orchestrator/seededFindingReader.ts; onBatchMinted hook → humanPrerequisiteSink; workflow-shaped proposal deps → agentThread/proposalExecutorWorkflowDeps.ts
