...
const FROZEN_LINE_CAPS: Record<string, number> = {
  'orchestrator/mcpServer/mcpQueryHandler.ts': 1555, // steps 1-3 + 9-14: type contract → mcpQueryMessages.ts; workflow-config + global-agent read/propose tools + verify/eval tools + interactive shell/OMP approval hooks + run-bound task/backlog tools + review-queue tools + artifact/design tools + sprint lane/batch tools → handlers/; mcp-queue → handlers/globalAgentToolHandlers.ts (TASK-293); backlog projections → backlogProjection.ts
