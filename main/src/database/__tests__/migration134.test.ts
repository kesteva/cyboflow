/**
 * Migration 134_approved_designs_flow_source.sql — `approved_designs` accepts
 * FLOW-sourced rows.
 *
 * Proves:
...
const MIGRATION_134 = '134_approved_designs_flow_source.sql';
...
function apply133(db: Database.Database): void {
  for (const statement of splitSqlStatements(readMigration(MIGRATION_134))) {
    try {
      db.exec(statement);
    } catch (err) {
