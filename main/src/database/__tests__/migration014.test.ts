/**
 * Migration 014_native_tasks.sql — schema + seed integration tests.
 *
 * Applies 006_cyboflow_schema.sql then 014_native_tasks.sql against an
...
function buildDb(): Database.Database {
  ...
  db.exec(sql006);
  const sql014 = readFileSync(join(__dirname, '..', 'migrations', '014_native_tasks.sql'), 'utf-8');
  db.exec(sql014);

  return db;
}

describe('Migration 014: native task backlog schema', () => {
