/**
 * Migration 142: widen agent_proposals.kind to admit 'start-quick-session' (the
 * global assistant's start-a-quick-session-with-a-brief proposal, TASK-295).
 *
 * Same full-recreate recipe as 125, 138 and 141, with the same hazard to pin:
...
const MIGRATION_142 = readFileSync(join(MIGRATIONS, '142_agent_proposal_start_quick_session_kind.sql'), 'utf-8');
...
const PRIOR_KINDS = ['launch-run', 'reprioritize-backlog', 'edit-workflow', 'open-session', 'create-backlog-items', 'create-workflow', 'triage-findings'];
...
describe("Migration 142: agent_proposals.kind admits 'start-quick-session'", () => {
  it('rejects the new kind BEFORE the migration and accepts it after', () => {
    const db = buildDb();
    expect(() => insertProposal(db, 'p-before', 'start-quick-session')).toThrow(/CHECK constraint failed/);

    db.exec(MIGRATION_142);
    insertProposal(db, 'p-after', 'start-quick-session');
    expect(db.prepare('SELECT kind FROM agent_proposals WHERE id = ?').get('p-after')).toEqual({
      kind: 'start-quick-session',
    });
  });

  it('keeps the seven pre-existing kinds valid and still rejects an unknown one', () => {
    const db = buildDb();
    db.exec(MIGRATION_142);
  ...
  it('keeps the ON DELETE CASCADE to agent_threads after the rebuild', () => {
    ...
    db.exec(MIGRATION_142);
    db.pragma('foreign_keys = ON');
    insertProposal(db, 'p-cascade', 'start-quick-session');
