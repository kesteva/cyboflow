/**
 * Migration 060_compound_recommendations_atype.sql — schema + constraint tests.
 *
 * Applies 006 -> 011 -> 014 -> 015 -> 016 -> 035 -> 045 -> 060 against an
...
function buildDb(): Database.Database {
  ...
  seedProject(db);
  apply(db, [...THROUGH_045, '060_compound_recommendations_atype.sql']);
  return db;
}
...
describe('Migration 060: compound-recommendations artifact atype', () => {
  ...
  it('preserves pre-existing artifacts rows across the copy', () => {
    const db = new Database(':memory:');
    seedProject(db);
    apply(db, THROUGH_045); // up to but NOT including 060
    seedRun(db, 'run-keep');
    db.prepare(
    ...
    apply(db, ['060_compound_recommendations_atype.sql']);
