/**
 * Integration tests for migrations 059-064 provider/runtime columns.
 *
...
describe('Migrations 059-064: agent provider/runtime columns', () => {
  ...
  it('each column migration has the expected duplicate-column idempotency signal', () => {
    const db = baseDb();
    db.exec(readMigration('059_session_agent_provider.sql'));
    expect(() => db.exec(readMigration('059_session_agent_provider.sql'))).toThrow(
  ...
  it('the backfill file is idempotent after all columns exist', () => {
    const db = baseDb();
    applyProviderRuntimeMigrations(db);
    expect(() => db.exec(readMigration('064_agent_provider_runtime_backfill.sql'))).not.toThrow();
    expect(
      db.prepare('SELECT agent_provider, agent_runtime FROM workflow_runs WHERE id = ?').get('wr-pty'),
    ).toEqual({ agent_provider: 'claude', agent_runtime: 'claude-interactive' });
    db.close();
