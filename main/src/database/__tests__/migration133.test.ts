/**
 * Migration 133_custom_views.sql — `custom_views`, `custom_widgets`, and the
...
const MIGRATION_133 = readFileSync(join(__dirname, '..', 'migrations', '133_custom_views.sql'), 'utf-8');

let tmpDir: string;
let dbPath: string;

beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'cyboflow-migration133-'));
...
describe('Migration 133: Custom Views tables', () => {
  ...
  it('is idempotent: re-applying 133 directly is a no-op (same columns, existing rows untouched)', () => {
    const svc = new DatabaseService(dbPath);
    svc.initialize();
    const raw = svc.getDb();
    ...
    expect(() => raw.exec(MIGRATION_133)).not.toThrow();
