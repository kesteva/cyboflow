/**
 * Migration 136_adversarial_review_atype.sql — schema + constraint tests.
 *
 * Applies the artifacts chain 006 -> … -> 073 -> 088 -> 089 -> 091 -> 097 -> 099
 * -> 102 -> 136 against an in-memory SQLite instance (mirroring
...
/** Every atype the CHECK must admit after 136. */
const ALL_ATYPES = [
  'idea-spec',
  'decomposed-stories',
  'screenshots',
...
function buildDb(): Database.Database {
  ...
  seedProject(db);
  apply(db, [...THROUGH_102, '136_adversarial_review_atype.sql']);
  return db;
}
...
describe('Migration 136: adversarial-review artifact atype', () => {
  ...
  it('(d) preserves pre-existing artifacts rows across the copy (full column shape)', () => {
    const db = new Database(':memory:');
    seedProject(db);
    apply(db, THROUGH_102); // up to but NOT including 136
    seedRun(db, 'run-keep');
    db.prepare(
    ...
    apply(db, ['136_adversarial_review_atype.sql']);
    ...
    expect(row).toMatchObject({
      ...
      // 088-era column carried through the 136 copy, not re-defaulted to 1.
      revision: 3,
    });
    db.close();
  });
  ...
  it('(f) the fresh-DB initialize() path includes adversarial-review in the atype CHECK', () => {
    ...
    try {
      ...
      db.prepare(`INSERT INTO projects (id, name, path) VALUES (1, 'Proj', '/tmp/proj-136')`).run();
      db.prepare(
