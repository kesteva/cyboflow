/**
 * Migration 107_agent_runtime_omp_fleet.sql — admitting 'omp-fleet'.
 *
 ...
 * by a DatabaseService whose migrations dir omits 107 ONLY (103 and 104 are
 ...
 *      107 does not widen, and a bogus runtime is still rejected everywhere.
 *   7. The fresh-install path lands the same constraints.
 *   8. Re-applying 107 after a cleared ledger marker is a harmless no-op.
 */
...
const MIGRATION_107 = '107_agent_runtime_omp_fleet.sql';

let tmpDir: string;
let dbPath: string;

beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'cyboflow-migration107-'));
...
/**
 * A migrations dir holding every real migration EXCEPT 107 — i.e. a shipped
 ...
 */
function migrationsDirWithout107(): string {
  const dir = join(tmpDir, 'migrations-pre-107');
  mkdirSync(dir);
  for (const name of readdirSync(MIGRATIONS_DIR)) {
    if (name === MIGRATION_107) continue;
...
function seedRows(db: BetterSqlite3.Database): void {
  db.prepare(`INSERT INTO projects (id, name, path) VALUES (1, 'Proj', '/tmp/p107')`).run();
  db.prepare(
...
/** Migrate to the shipped pre-107 state, seed, snapshot; then boot with 107. */
function upgradeThrough107(): { db: BetterSqlite3.Database; before: Snapshot; svc: DatabaseService } {
  const pre107 = migrationsDirWithout107();
  // Two pre-107 boots, not one: initializeSchema() reads PRAGMA table_info(sessions)
  ...
  // launch. Settling that here keeps the snapshot diff about 107 alone — and
  // makes `status_message` present in the pre-state, which is the whole point.
  openAt(pre107).close();
  const pre = openAt(pre107);
  seedRows(pre.getDb());
...
describe('Migration 107: omp-fleet admitted on the session + workflow-run runtime CHECKs', () => {
  it('(a) makes omp-fleet storable on sessions and workflow_runs', () => {
    const { db, svc } = upgradeThrough107();
  ...
  it('(c) preserves every pre-existing row verbatim, including the omp-sdk session', () => {
    const { db, before, svc } = upgradeThrough107();
    for (const t of TARGET_TABLES) {
  ...
  it('(d) loses no column — status_message (added imperatively, listed in no .sql) survives', () => {
    ...
    expect(columnNames(db, 'sessions')).not.toContain('agent_runtime_widen_107');
    expect(columnNames(db, 'workflow_runs')).not.toContain('agent_runtime_widen_107');
    svc.close();
  });
  ...
  it('(g) lands the same constraints on a fresh install', () => {
    const svc = openAt(MIGRATIONS_DIR);
    const db = svc.getDb();
    db.prepare(`INSERT INTO projects (id, name, path) VALUES (1, 'Proj', '/tmp/p107f')`).run();
    db.prepare(
      `INSERT INTO workflows (id, project_id, name, spec_json) VALUES ('wf-1', 1, 'sprint', '{}')`,
    ).run();
    expect(trySession(db, 's-fleet', 'omp', 'omp-fleet')).toBeNull();
  ...
  it('(h) re-applying 107 after a cleared ledger marker is a harmless no-op', () => {
    const { db, before, svc } = upgradeThrough107();
    db.prepare('DELETE FROM user_preferences WHERE key = ?').run(
      `file_migration_applied:${MIGRATION_107}`,
    );
    svc.close();
