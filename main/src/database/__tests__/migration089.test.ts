 ...
 *   1. 'interactive-prototype' is insertable alongside every pre-existing atype;
 *      a bogus atype is still rejected by the widened CHECK.
 *   2. The `revision` column (added by 082) survives the 084 recreate — the row
 *      keeps its value and the default is preserved.
 *   3. interactive-prototype is one-per-(run, atype) (NOT per-entity).
...
describe('Migration 089: interactive-prototype atype', () => {
  ...
  it('(b) the revision column (082) survives the 084 recreate with its default', () => {
    const db = buildDb();
    seedRun(db, 'run-1');
    insertArtifact(db, 'art_rev', { atype: 'interactive-prototype' });
  ...
  it('(c) interactive-prototype is one-per-(run, atype) (NOT per-entity)', () => {
    const db = buildDb();
    seedRun(db, 'run-1');
    expect(() =>
      insertArtifact(db, 'ip_1', { atype: 'interactive-prototype', sourceRef: 'x' }),
    ).not.toThrow();
    // A second interactive-prototype in the SAME run — even with a different
    // source_ref — collides on idx_artifacts_one_per_atype.
    expect(() =>
      insertArtifact(db, 'ip_2', { atype: 'interactive-prototype', sourceRef: 'y' }),
    ).toThrow(/UNIQUE/i);
    db.close();
  });
  ...
  it('(d) preserves pre-existing artifacts rows (incl. revision) across the copy', () => {
    ...
    apply(db, THROUGH_082);
    db.exec('CREATE TABLE sessions (id TEXT PRIMARY KEY)');
    apply(db, ['082_design_mode_v0.sql']); // up to but NOT including 084
    seedRun(db, 'run-keep');
    db.prepare(
  ...
  it('(f) the fresh-DB initialize() path includes interactive-prototype + the split indexes', () => {
    ...
    try {
      ...
      db.prepare(`INSERT INTO projects (id, name, path) VALUES (1, 'Proj', '/tmp/proj-084')`).run();
      db.prepare(
      ...
      expect(() =>
        db
          .prepare(
            `INSERT INTO artifacts (id, run_id, atype, label, mode) VALUES ('art_fresh', 'run-1', 'interactive-prototype', 'Interactive', 'canvas')`,
          )
          .run(),
      ).not.toThrow();
