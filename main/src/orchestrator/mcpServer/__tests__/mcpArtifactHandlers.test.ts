...
function buildDb(): Database.Database {
  ...
  // artifacts.revision (migration 082) — the enrich-in-place revision bump; the
  // idempotent-re-report tests trigger it. Added directly since this DB
  // hand-picks a migration subset predating 082.
  db.exec('ALTER TABLE artifacts ADD COLUMN revision INTEGER NOT NULL DEFAULT 1');
  // A minimal Crystal-legacy `sessions` table (with the migration-082
