 ...
 * must NEVER be attached proactively, and `switch_to_orchestrated` is single-turn too:
 * an EXPLICIT user request for the handover is attached on that same turn (the reply
 * carries the one-way + separate-runtime warning), and the suggest-first-in-reply wait
 * applies only when the model proposes the handover unprompted — both keep their
 * existing explicit-ask contracts. The host (not the monitor) validates run state and executes
...
- Retrying / handover:
  ...
  - Action "switch_to_orchestrated": hand the ENTIRE run over to a full interactive agent that continues the remaining workflow conversationally. Offer this ONLY when the user's request cannot be served by "retry_step" or by simply answering — a freeform intervention such as "fix the conflict by hand then continue" or "change the approach for the remaining steps". NEVER attach it merely because a retry failed. It is ONE-WAY — the run does NOT return to step-by-step execution afterward. When the user EXPLICITLY asks for the handover (naming it, or plainly asking you to take the run over / continue it conversationally), ATTACH IT ON THAT SAME TURN: do NOT ask them to confirm a request they just made. State the consequences once, in the reply that hands over — that it is one-way, and that steps configured to run in a separate runtime (e.g. a Codex-pinned step) will be switched to this one, since the single handover agent runs the entire remaining workflow itself. Only when YOU are proposing the handover UNPROMPTED do you SUGGEST it in prose with that same warning and WAIT for the user's EXPLICIT confirmation on a later turn before attaching it. When you do attach it, set \`reason\` to a faithful 1-3 sentence summary of the user's outstanding request (what they want done after the handover). The HOST validates the run's state and executes the handover — you never claim it succeeded yourself.
 ...
 * keeps its own prompt contract (an explicit user request is attached on that same turn;
 * only an unprompted offer is made in prose and waits for confirmation before attaching). Both were live-verified
