/**
 * builtInWorkflows — the in-repo, self-contained source of the three user-facing
 * cyboflow workflows (Planner + Sprint + Compound), each seeded as a `workflows`
 * row so the registry has a row to launch by `wf-<projectId>-<name>`. A parallel
 * sprint is a single session-hosted `sprint` run (the orchestrator agent fans out
 ...
 * repo as sibling `.md` files (`planner.md`, `sprint.md`, `compound.md`), and
 * `workflow_path` points at them so `WorkflowRegistry.seed()` can read each
 * file's frontmatter `permission_mode`.
 *
...
/**
 * Build the in-repo built-in workflow descriptors (Planner + Sprint + Compound),
 * each pointing at its sibling prompt `.md` file resolved relative to the
 * compiled bundle.
 *
