...
export class TaskChangeRouter {
   ...
   * Calls the SAME exported helper the read/list path uses (single-id array)
   * so the emit path and the full-read path can never disagree on which
   * batches/experiments count or how a label is derived — the only thing that
   * differs is the CARDINALITY (one task per event vs a whole listing's
   * worth). This delegation is what the "emit-path SHAPE PARITY" test below
   * verifies stays true.
   */
  private gatherMemberships(taskId: string): BacklogMembership[] {
    return loadMembershipsForTaskIds(this.db, [taskId]).get(taskId) ?? [];
  }
