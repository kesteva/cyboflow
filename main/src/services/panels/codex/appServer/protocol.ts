 ...
 * 0.156.1. Keep this file version-pinned until generated bindings replace it.
 */
...
export interface AppServerInitializeCapabilities {
  ...
  /** Legacy opt-in for the `openai/form` MCP extension; superseded by `extensions`. */
  mcpServerOpenaiFormElicitation?: boolean;
  optOutNotificationMethods?: string[] | null;
  /** MCP extension settings declared by the client. cyboflow declares none today. */
  extensions?: Record<string, AppServerJsonValue> | null;
}
...
export interface AppServerTurnStartParams {
  ...
  toolOutput?: {
    ...
    // Wire type is `FunctionCallOutputBody` (`string | FunctionCallOutputContentItem[]`),
    // deliberately widened here since cyboflow never populates this field.
    output: AppServerJsonValue;
  } | null;
...
/**
 * Public explanation attached to a turn error the model refused on alignment
 * grounds. `steer` is the continuation instruction the TUI resubmits; cyboflow
 * only surfaces the explanation, so it stays opaque JSON here.
 */
export interface TurnErrorMisalignment {
  errorType: string | null;
  detailedExplanation: string | null;
  steer: AppServerJsonValue | null;
}

/** Follow-up an async agent message asks the human, alongside its text. */
...
export interface CommandExecutionRequestApprovalParams {
  /** Optional here because the app-server docs default it to `command` for older servers. */
