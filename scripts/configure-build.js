...
// The "Cyboflow Dev" variant's app icon, relative to the repo root (the form
// electron-builder resolves paths in). Generated by scripts/generate-icons.sh.
const DEV_ICON_ICNS = 'main/assets/icon-dev.icns';
const DEV_ICON_ICO = 'main/assets/icon-dev.ico';

/**
 * Fail the build when a dev icon is missing rather than letting electron-builder
 * silently substitute its stock Electron icon — a dev DMG that looks like the
 * stable app (or like a generic Electron app) defeats the point of the variant.
 */
function assertDevIconsExist(isWin) {
  const required = isWin ? [DEV_ICON_ICO] : [DEV_ICON_ICNS];
  for (const rel of required) {
    if (!fs.existsSync(path.join(__dirname, '..', rel))) {
      console.error(`Error: dev variant requires ${rel}; run scripts/generate-icons.sh`);
      process.exit(1);
    }
  }
}
...
function configureBuild() {
  ...
  if (isDev) {
    ...
    // The dev icon is the same mark with a blue accent instead of the stable
    // orange, so the two apps are told apart at a glance in the Dock / taskbar.
    // Both files are generated by scripts/generate-icons.sh alongside the stable
    // pair; assertDevIconsExist below refuses to build without them, because
    // electron-builder would otherwise quietly fall back to its own default icon.
    config.mac.icon = DEV_ICON_ICNS;
    if (isWin && config.win) {
      config.win.artifactName = 'Cyboflow-Dev-${version}-Windows-${arch}.${ext}';
      config.win.icon = DEV_ICON_ICO;
    }
    assertDevIconsExist(isWin);
